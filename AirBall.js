...
function mottaTaster(e){
  if (e.key === " "){
    ...
    antHopp+=1;
    trykk+=1;
    strek= new sprettBrett(450, spiller1.y, hinderfart, "black");
    }
}
...
function ferdig(){
      ...
      gameover.innerHTML+="Average jumps per obstacle " + gjennomsnitt;
      gameover.innerHTML+="<h1> You passed " + p + " obstacles with " + antHopp + " jumps</h1><br><h3>Press ENTER or R to restart</h3>";
...
class spiller {
    ...
    flytt(){
      ...
      if(this.y>=600-radius){
        ...
        this.y+= -this.fart + tyngdekraft*Math.pow((hangtime+1), 2);
      }else{
        hangtime = ((klikktime - new Date())/1000)*-1;
        this.y+= -this.fart + tyngdekraft*Math.pow((hangtime+1), 2);
